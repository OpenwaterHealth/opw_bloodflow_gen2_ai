...
import torch, platform, datetime, glob, math
...
def run_fold(X_train, X_test, Y_train, Y_test, fold):
    ...
    if is_debug:
        ...
        summaryPath = 'bloodflow/layer98lg_'+str(fold+1)+'fold_'+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    #Check if best model path exists; add dictionary filter in case model has changed
    bestModelPaths = ['../../bestModel/model.pt', '../bestModel/model.pt']
    modelLoaded = False
    for bestModelPath in bestModelPaths:
        if glob.glob(bestModelPath):
            modelLoaded = True
            print('Loading model from {}'.format(bestModelPath))
            pretrained_dict = torch.load(bestModelPath)
            model_dict = model.state_dict()
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
            model_dict.update(pretrained_dict) 
            model.load_state_dict(model_dict)
            print('Pretrained model, freezing following model components')
    ...
    for ep in tqdm(range(n_epoch), desc="epoch", leave=False):
        ...
        #Decrease the model dropout every 8th epoch
        #if not ep%8:
            # Use the tanh function between -3 to + 3 dropout will be close to 1 at -3 and close to 0 at +3
        #    phase = int(ep/8)-3.0
        #    dropRate = 1-(math.tanh(phase)+1)/2.0
        #    print('Setting dropout to {} in epoch {}'.format(dropRate,ep))
        #    model.do.p = dropRate
        #    writer.add_scalar('DropRate', dropRate, step)
...
if __name__=="__main__":
    ...
    pool.close()
    pool.join()
    run_fold, args=(X_train_folds[4], X_test_folds[4], Y_train_folds[4], Y_test_folds[4], 4)
    '''
    for _ in range(2):
        for i in range(5):
