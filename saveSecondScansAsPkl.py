...
#%%
# Read in the data
LVOsFilter = [('scansValid','True'), ('lowNoise','True'), ('subjectType','LVO'), ('subjectState','SecondScan')]
IHCsFilter = [('scansValid','True'), ('lowNoise','True'), ('subjectType','IHC'), ('subjectState','SecondScan')]
...
redcapData = redcapData[['subject_id', 'source', 'age', 'race_score', 'pre_op_sbp', 'pre_op_dbp']]
...
allScans = []
allTime = 0
if os.path.exists('allSecondScansDL.pkl'):
    if os.path.getmtime('allSecondScansDL.pkl') > os.path.getmtime(scanIndexJson):
        print('Loading allSecondScansDL.pkl')
        with open('allSecondScansDL.pkl', 'rb') as f:
            allScans = pickle.load(f)
if len(allScans) == 0:
    print('allSecondScansDL.pkl is older than ScanIndex.json, regenerating allSecondScansDL.pkl')
    ...
    with open('allSecondScansDL.pkl', 'wb') as f:
...
# %%
# Save the data
with open('secondScanData.npy', 'wb') as f:
    np.save(f, scanData)
with open('secondClassData.npy', 'wb') as f:
    np.save(f, classData)
with open('secondSubjectID.npy', 'wb') as f:
    np.save(f, subjectID)
# %%
